...
public class CheckoutSaga {
    ...
    public void executeCheckout(String orderId, String paymentToken, String discountCode) {
        ...
        BuyerSnapshot buyerSnapshot = new BuyerSnapshot(resolveBuyerName(order.getBuyerRef().memberId()));
        
        List<PurchasedItem> purchasedItems = order.getItems().stream()
                .map(this::toPurchasedItem)
    ...
    /**
     * Builds a purchase-record line item using human-readable names rather than the raw
     * UUIDs stored on the order. Falls back to the id if the zone/seat can't be resolved so
     * the receipt is never left blank.
     */
    private PurchasedItem toPurchasedItem(OrderItem item) {
        Zone zone = zoneRepository.findById(new ZoneId(item.getZoneId())).orElse(null);
        String zoneName = (zone != null && zone.zoneName() != null && !zone.zoneName().isBlank())
                ? zone.zoneName()
                : item.getZoneId();
        String seatLabel = resolveSeatLabel(zone, item.getSeatId());
        return new PurchasedItem(zoneName, seatLabel, item.getQuantity(), item.getUnitPrice());
    }

    private String resolveSeatLabel(Zone zone, String seatId) {
        if (seatId == null) {
            return null;
        }
        if (zone != null) {
            for (Row row : zone.rows()) {
                for (Seat seat : row.seats()) {
                    if (seat.seatId().value().equals(seatId)) {
                        return "Row " + seat.rowLabel() + ", Seat " + seat.seatNumber();
                    }
                }
            }
        }
    ...
    private String resolveBuyerName(String memberId) {
        if (memberId == null) {
            return "Guest";
        }
        ...
        for (OrderItem item : items) {
            ...
            zoneRepository.withLock(zId, () -> {
                zoneRepository.findById(zId).ifPresent(zone -> {
                    // Best-effort compensation: a seat may already have been released
                    // (e.g. by a prior failed attempt or the expiry sweep). Don't let a
                    // compensation failure mask the original checkout failure.
                    try {
                        zone.releaseSeat(new SeatId(item.getSeatId()));
                        zoneRepository.save(zone);
                    } catch (RuntimeException ex) {
                        logger.warn("Compensation could not release seat {} in zone {}: {}",
                                item.getSeatId(), item.getZoneId(), ex.getMessage());
                    }
                });
            });
        }
