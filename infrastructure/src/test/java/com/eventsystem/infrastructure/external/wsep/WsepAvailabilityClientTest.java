...
class WsepAvailabilityClientTest {
    ...
    void isAvailable_whenHttpFails_throwsCommunicationException() throws Exception {
        ...
        // REQ: SYS-01, SYS-03, SYS-04, UC 1, UAT-01 - initialization can verify WSEP availability.
    @Test
    void areExternalSystemsAvailable_whenHandshakeReturnsOk_returnsTrue() throws Exception {
        try (WsepTestServer server = new WsepTestServer()) {
            server.enqueue(200, "OK");
            ...
            assertTrue(client.areExternalSystemsAvailable());
    ...
    // REQ: SYS-01, ROB-01, UC 1, UAT-02 - unexpected handshake response means unavailable.
    @Test
    void areExternalSystemsAvailable_whenHandshakeReturnsUnexpectedBody_returnsFalse() throws Exception {
        try (WsepTestServer server = new WsepTestServer()) {
            server.enqueue(200, "NO");
            ...
            assertFalse(client.areExternalSystemsAvailable());
        }
    }

    // REQ: ROB-01, TST-17, UC 1, UAT-02 - external service down is communication failure.
    @Test
    void areExternalSystemsAvailable_whenHttpFails_throwsCommunicationException() throws Exception {
        try (WsepTestServer server = new WsepTestServer()) {
            server.enqueue(503, "down");
            ...
            assertThrows(WsepCommunicationException.class, () -> {client.areExternalSystemsAvailable();});
        }
    }
}